...
impl<W> DataAccess<W> where W: Write {
    ...
    pub fn set<K: Into<String>, V: IntoTag>(&mut self, key: K, value: V) -> anyhow::Result<()> {
        ...
        self.writer.lock().unwrap().write_line(format!("data modify storage {} {} set value {}", self.name, k, v.stringify())).map_err(|it| anyhow::Error::from(it))?;
